This project is an end-to-end Sales Data Warehouse and Analytics Platform designed to consolidate, model, and analyze sales data across multiple sources. The solution covers the full data lifecycle—from ingestion and transformation to modeling, KPI tracking, and interactive dashboarding.
## 📌 Business Problems

The company holds large volumes of sales data, but key insights are difficult to obtain due to the lack of a unified analytical system. Comparing performance across branches, cities, and product categories is challenging, making it hard for management to evaluate growth and profitability.

Customer purchasing behaviour is not clearly understood, as the company cannot analyze buying patterns by age group, gender, city, or loyalty level. Measuring essential KPIs such as monthly revenue, quantity sold, discount amounts, profit, and supplier contribution is also difficult due to fragmented data.

Additionally, historical changes—such as updates to product prices, customer loyalty levels, and branch managers—are not consistently tracked, limiting the organization’s ability to perform trend analysis or audit past changes.

## 📌 Project Overview

This project builds a modern Sales Data Warehouse and analytics platform to solve these issues, delivering a centralized, accurate, and scalable environment for reporting, KPI measurement, and data-driven decision-making. It integrates sales, inventory, customer, and operational data into a single source of truth, allowing stakeholders to access consistent metrics and standardized reporting across all branches and product categories.


## 📌 Business Questions

The Data Warehouse is designed to answer critical business questions across Sales, Inventory, Customers, and Operations:

### 🔹 Sales Performance
- How do branches and cities compare in revenue, profit, and growth?
- Which product categories perform best in terms of quantity sold and profit?
- What are the monthly trends for revenue, profit, and units sold?
- Which suppliers contribute most to overall performance?

### 🔹 Customer Analytics
- What is customer purchasing behaviour by age group, gender, city, and loyalty level?
- Which customer segments generate the highest revenue or lifetime value?
- How do discounts influence customer behaviour and basket size?

### 🔹 Operational Insights
- How have product prices changed over time, and what impact did these changes have on sales?
- How do changes in branch managers affect branch performance?
- How do customer loyalty upgrades or downgrades trend over time?

